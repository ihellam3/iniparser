...
void dictionary_merge(dictionary* src, dictionary* dest) {
    size_t i;
    char* val;
    if(dest->n < 1) {
        return;
    ...
    for(i = 0; i < dest->size; i++) {
        if(dest->key[i]) {
            val = dictionary_get(src, dest->key[i], INI_INVALID_KEY);
            if (INI_INVALID_KEY == val) {
                dictionary_set(src, dest->key[i], dest->val[i] ? dest->val[i] : NULL); 
            }
        }
    }
}
